...
NEW,CODE_VERIFIDE,DONE,PHOTO_DONE=('new','code_verifide','done','photo_done')
...
class CustomUser(BaseModel,AbstractUser):
    ...
    def check_username(self):
        if not self.username:
            ...
            while CustomUser.objects.filter(username=self.username).exists():
                self.username=f"{self.username+str(random.randint(1,100))}"
    ...
    def check_pass(self):
        if not self.password:
            self.password = f"pass{uuid.uuid4().__str__().split('-')[-2]}"
...
class CodeVerified(BaseModel):
    ...
    user=models.ForeignKey(CustomUser,on_delete=models.CASCADE,related_name='verify_cod')
    ...
    def save(self,*args,**kwargs):
        if self.verify_type==VIA_EMAIL:
            expiration_time=datetime.now()+timedelta(minutes=EXPIRATION_EMAIL)
        else:
